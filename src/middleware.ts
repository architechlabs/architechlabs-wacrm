...
function sanitizeAuthDiagnostic(
  value: string,
  maxLength = MAX_AUTH_ERROR_MESSAGE_LENGTH
) {
  return value
    .replace(/[\u0000-\u001f\u007f]+/g, ' ')
...
export async function middleware(request: NextRequest) {
  ...
  // PKCE callback authenticates by exchanging a one-use code, not by an
  // existing session. Preserve its verifier cookie for the route handler.
